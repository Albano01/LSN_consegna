...
{ 
  ...
  int nconf = 1;
  for(int iblk=1; iblk <= nblk+1; iblk++){ 			            //SIMULAZIONE
    Reset(iblk);   						                        //Reset block averages
    for(int istep=1; istep <= nstep; istep++){
      Move();                                               //Muovere particelle con algoritmo Verlet o Metropolis 
...
//        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"! 
    ...
    if(iblk!=1){                                //Scarto il primo blocco
    Averages(iblk-1);
...
//Read input informations
  ...
  cout << "è la temperatura iniziale che poi cambia: guarda file di output/grafici Python!"<<endl;
...
//Prepare arrays for measurements
  ...
  ie = 3; //Total energy
  ip = 4; //Pressione
  n_props = 5; //Number of observables                         //HO INTRODOTTO LA PRESSIONE ip = 4 e n_props=5
...
//Read initial configuration
  ...
  else 
  {
    ...
    for (int i=0; i<npart; ++i)
    {
      vx[i] = vx[i] - sumv[0];                          //Sottraggo drift velocity per particle (metto somma totale velocità uguale a zero ---> mom ang lineare totale è nullo)
      vy[i] = vy[i] - sumv[1];
      vz[i] = vz[i] - sumv[2];
...
void Move(){
  ...
  } else //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Molecular Dynamics (NVE) move!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  {
    ...
    for(int i=0; i<npart; ++i){ //Forza su particella i
      fx[i] = Force(i,0);
      fy[i] = Force(i,1);
      fz[i] = Force(i,2);
    }
...
double Force(int ip, int idir){ //Forza come -Grad_ip V(r)
  double f=0.0;
  double dvec[3], dr;

  for (int i=0; i<npart; ++i){
    if(i != ip){
      dvec[0] = Pbc( x[ip] - x[i] );  // distanza ip-i con pbc
      dvec[1] = Pbc( y[ip] - y[i] );
      dvec[2] = Pbc( z[ip] - z[i] );
...
void Measure() //Misura delle proprietà termodinamiche
{
  double v = 0.0, kin=0.0, ppp = 0;              //AGGIUNTO IO: ppp addendo del calcolo della pressione
  double vij, pij;                              //vij parziale per il calcolo del potenziale, AGGIUNTO IO: pij parziale per il calcolo della pressione
  double dx, dy, dz, dr;
  int counter=0;                                //Contatore per fare media nella formula pressione sotto!

//ciclo su coppie di particelle
  for (int i=0; i<npart-1; ++i){
    for (int j=i+1; j<npart; ++j){
      ...
      if(dr < rcut){
        pij = 1./pow(dr,12) - (1./2.)*(1./pow(dr,6));       //AGGIUNTO IO: calcolo addendo pressione
        ppp += pij;
        counter ++;
  ...
  walker[ip] = rho*walker[it] + (48./(3.*vol))*(ppp);      //Pressione
...
void Averages(int iblk){ //Print results for current block
   ...
   const int wd=12;
    ...
    stima_press = blk_av[ip]/blk_norm; //Pressione: FATTO IO
