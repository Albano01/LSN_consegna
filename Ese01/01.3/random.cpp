...
double Random :: Exp(double mean) {
   // This function generates a random number from a Exponential distribution with given mean (FATTO DA ME PER QUESTO ESERCIZIO)
   double s=Rannyu();
   double r = -log(1.- s);
	return r/mean;
}
...
double Random :: Lorentz(double mu, double Gamma) {
   // This function generates a random number from a Cauchy_Lorentz distribution with given mu (picco) and Gamma (fattore di scala) (FATTO DA ME PER QUESTO ESERCIZIO)
   double s=Rannyu();
